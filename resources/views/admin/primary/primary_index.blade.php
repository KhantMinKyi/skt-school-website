...
@section('content')
    ...
    <div class="card">
        <div class="card-header font-weight-bold h5">Activity Photos</div>
        <div class="text-right m-4 ">
            <a href="#" class="btn btn-success btn-md" data-bs-toggle="modal"
                data-bs-target="#modalNewPrimaryActivityPhoto">
                <div class="d-flex  align-items-center">
                    <span class="animated-icon mr-2 ">
                        <div style="width:14px;height:14px; cursor: pointer;"
                            data-animation-path="{{ asset('admin/vendor/animated-icons/add/add.json') }}"
            ...
            <div class="table-responsive">
                <div class="table-responsive">
                    <table class="table table-bordered table-striped">
                        ...
                        <tbody>
                            @foreach ($primary_activity_photos as $primary_activity_photo)
                                <tr>
                                    <td>{{ $primary_activity_photo->id }}</td>
                                    <td>{{ $primary_activity_photo->branch->branch_name }}</td>
                                    <td>
                                        {{-- <div id="pdfThumbnails" class=" inline-block w-full ">
                                            <div class="pdf-item flex flex-wrap">
                                                <canvas
                                                    class="pdf-thumbnail w-48 h-auto cursor-pointer shadow-lg rounded-lg m-4"
                                                    data-pdf="{{ asset($primary_activity_photo->primary_activity_file) }}">
                                                </canvas>
                                            </div>
                                        </div> --}}
                                        <img src="{{ asset($primary_activity_photo->primary_activity_file) }}"
                                            alt="" style="height:150px;">
                                    </td>
                                    <td>{{ $primary_activity_photo->primary_activity_title }}</td>
                                    <td class="">
                                        <div class="d-flex justify-content-center align-items-center">
                                            <a class="animated-icon mr-2"
                                                href="{{ route('admin-primary-activity-photos.edit', $primary_activity_photo->id) }}">
                                                <div style="width:14px;height:14px; cursor: pointer;"
                                                    data-animation-path="{{ asset('admin/vendor/animated-icons/edit/edit.json') }}"
                                                    data-anim-loop="false" title="Edit Primary Newsletter">
                                                </div>
                                            ...
                                            <form
                                                action="{{ route('admin-primary-activity-photos.destroy', $primary_activity_photo->id) }}"
                                                method="post">
                                                @csrf
                                                @method('DELETE')
                                                <button class="animated-icon mr-2 border-0 mt-1" type="submit">
                ...
                <form action="{{ route('admin-primary-newsletters.store') }}" method="post" enctype="multipart/form-data">
                    ...
                    <div class="modal-body">
                        <div class="row">
                            ...
                            <div class="col-12">
                                <div class="row mb-3">
                                    <label for="inputEmail3" class="col-3 col-form-label">Primary Newsletter Title
                                    </label>
                                    <div class="col-9">
    ...
    <div class="modal fade" tabindex="-1" id="modalNewPrimaryActivityPhoto">
        <div class="modal-dialog modal-xl modal-dialog-centered">
            <div class="modal-content">
                <form action="{{ route('admin-primary-activity-photos.store') }}" method="post"
                    enctype="multipart/form-data">
                    @csrf
                    <div class="modal-header">
                        <h5 class="modal-title">Add New Primary Activity Photos</h5>
                        <button type="button" class="btn btn-icon btn-sm btn-active-light-primary ms-2"
                            data-bs-dismiss="modal" aria-label="Close">
                            <span class="svg-icon svg-icon-2x"><i class="fa-solid fa-xmark"></i></span>
                        </button>
                        ...
                        <div class="row">
                            <div class="col-12">
                                <div class="row mb-3">
                                    <label for="inputEmail3" class="col-3 col-form-label"> Branch</label>
                                    <div class="col-sm-9">
                                        <select class="form-control select2" name="primary_activity_branch_id" required
                                            title="Branch is required">
                                            <option disabled selected>Select a Branch</option>
                                            @foreach ($branches as $key => $data)
                                                <option value='{{ $data->id }}'>{{ $data->branch_name }}</option>
                            ...
                            <div class="col-12">
                                <div class="row mb-3">
                                    <label for="inputEmail3" class="col-3 col-form-label"> Files </label>
                                    <div class="col-9">
                                        {{-- <small class="text-danger">Photo Size Must be 1024x1084</small> --}}
                                        <input type="file" placeholder="Choose Files " value=""
                                            name="primary_activity_files[]" title="File is required" required multiple
                                            class="form-control">
                                    </div>
                                </div>
                            </div>
                            <div class="col-12">
                                <div class="row mb-3">
                                    <label for="inputEmail3" class="col-3 col-form-label">Primary Activity Title
                                    </label>
                                    <div class="col-9">
                                        <input type="text" class="form-control"
                                            placeholder="Enter Primary Activity Title " required
                                            name="primary_activity_title">
                                    </div>
                                </div>
                            </div>
                        </div>
