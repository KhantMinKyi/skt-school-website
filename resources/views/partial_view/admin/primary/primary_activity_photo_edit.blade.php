...
@section('content')
    <div class="card">
        <div class="card-header font-weight-bold h5">Edit Activity Photo - {{ $primary_activity->primary_activity_title }}
        </div>
        <form action="{{ route('admin-primary-activity-photos.update', $primary_activity->id) }}" method="post"
                        ...
                        <div class="row mb-3">
                            ...
                            <div class="col-6">
                                <input type="file" placeholder="Choose File "
                                    value="{{ $primary_activity->primary_activity_file }}" name="primary_activity_file"
                                    title="File is required" class="form-control">
                            </div>
                            <div class="col-3">
                                {{-- <div id="pdfThumbnails" class=" inline-block w-full ">
                                    <div class="pdf-item flex flex-wrap">
                                        <canvas class="pdf-thumbnail w-48 h-auto cursor-pointer shadow-lg rounded-lg m-4"
                                            data-pdf="{{ asset($primary_activity->primary_activity_file) }}">
                                        </canvas>
                                    </div>
                                </div> --}}
                                <img src="{{ asset($primary_activity->primary_activity_file) }}" style="height: 150px"
                                    alt="">
                            </div>
                        </div>
                    </div>
                                ...
                                <select class="form-control select2" name="primary_activity_branch_id" required
                                    ...
                                    @foreach ($branches as $key => $data)
                                        <option value='{{ $data->id }}'
                                            @if ($primary_activity->primary_activity_branch_id === $data->id) selected @endif>{{ $data->branch_name }}
                                        </option>
                                    @endforeach
                                </select>
                            </div>
                        ...
                        <div class="row mb-3">
                            ...
                            <div class="col-9">
                                <input type="text" placeholder="Choose Name "
                                    value="{{ $primary_activity->primary_activity_title }}" name="primary_activity_title"
