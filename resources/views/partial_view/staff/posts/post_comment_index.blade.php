...
@section('content')
    ...
    <div class="d-flex justify-content-end m-4 ">
        <a href="{{ route('staff-posts.index') }}" class="animated-icon d-flex justify-content-center align-items-center">
                        ...
                        <tbody>
                            @foreach ($post_comments as $comment)
                                <tr>
                                    ...
                                    <td>{{ $comment->post_comment_subject }}</td>
                                    <td>{{ $comment->post_comment_body }}</td>
                                    <td> Created - {{ date('d-m-Y h:s', strToTime($comment->created_at)) }}
                                    <td class="text-center">
                                        @if ($comment->post_comment_status == 'approved')
                                            <span class="badge badge-success p-2">Approved</span>
                                        @elseif($comment->post_comment_status == 'pending')
                                    ...
                                    <td class="">
                                        <div class="d-flex justify-content-center align-items-center">
                                            <form action="{{ route('staff-post-comments.change_comment_status') }}"
                                                method="post">
                                                @csrf
                                                @method('PUT')
                                                <input type="hidden" name="post_comment_id" id=""
                                                    value="{{ $comment->id }}">
                                                <select name="post_comment_status" class="form-control comment-status"
                                                    data-comment-id="{{ $comment->id }}">
                                                    <option value="canceled"
                                                        @if ($comment->post_comment_status == 'canceled') selected @endif>Canceled
                                                    </option>
                                                    <option value="pending"
                                                        @if ($comment->post_comment_status == 'pending') selected @endif>Pending
                                                    </option>
                                                    <option value="approved"
                                                        @if ($comment->post_comment_status == 'approved') selected @endif>Approved
                                                    </option>
                                                </select>
                                            </form>
                                            <form action="{{ route('staff-post-comments.destroy', $comment->id) }}"
